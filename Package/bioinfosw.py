...
quercusTOA (Quercus Taxonomy-oriented Annotation).
...
class FormInstallBioinfoSoftware(QWidget):
    ...
    def pushbutton_execute_clicked(self):
        ...
        elif self.software_code == genlib.get_quercustoa_env_code():
        ...
        if OK:
            ...
            if self.software_code == genlib.get_miniforge3_code():
                ...
                process = dialogs.DialogProcess(self, self.head, self.install_quercustoa_env)
                process.exec()
                ...
                process = dialogs.DialogProcess(self, self.head, self.install_bioconda_env, self.software_code, software_dict, additional_library_list)
                process.exec()
            elif self.software_code == genlib.get_liftoff_code():
                software_dict = {'conda_code': genlib.get_liftoff_conda_code(), 'env_name': genlib.get_liftoff_environment(), 'version': version}
                additional_library_list = []
                process = dialogs.DialogProcess(self, self.head, self.install_bioconda_env, self.software_code, software_dict, additional_library_list)
                process.exec()
            elif self.software_code == genlib.get_liftofftools_code():
                software_dict = {'conda_code': genlib.get_liftofftools_conda_code(), 'env_name': genlib.get_liftofftools_environment(), 'version': version}
                additional_library_list = []
                process = dialogs.DialogProcess(self, self.head, self.install_bioconda_env, self.software_code, software_dict, additional_library_list)
                process.exec()
            elif self.software_code == genlib.get_mafft_code():
                software_dict = {'conda_code': genlib.get_mafft_conda_code(), 'env_name': genlib.get_mafft_environment(), 'version': version}
                additional_library_list = ['pymsaviz', 'dendropy']
                process = dialogs.DialogProcess(self, self.head, self.install_bioconda_env, self.software_code, software_dict, additional_library_list)
                process.exec()
   ...
   #---------------
    ...
    def install_quercustoa_env(self, process):
        '''
        ...
        if OK:
            process.write(f'{genlib.get_separator()}\n')
            process.write('Determining the run directory ...\n')
            current_run_dir = genlib.get_current_run_dir(result_dir, genlib.get_result_installation_subdir(), genlib.get_quercustoa_env_code())
            command = f'mkdir -p {current_run_dir}'
            rc = genlib.run_command(command, process, is_script=False)
            if rc == 0:
                process.write(f'The directory path is {current_run_dir}.\n')
        ...
        if OK:
            process.write(f'{genlib.get_separator()}\n')
            script_name = f'{genlib.get_quercustoa_env_code()}-installation.sh'
            process.write(f'Building the installation script {script_name} ...\n')
            (OK, _) = self.build_quercustoa_env_installation_script(temp_dir, script_name, current_run_dir)
            if OK:
                process.write('The file is built.\n')
            else:
                process.write('*** ERROR: The file could not be built.\n')
        ...
        if OK:
            process.write(f'{genlib.get_separator()}\n')
            starter_name = f'{genlib.get_quercustoa_env_code()}-installation-starter.sh'
            process.write(f'Building the process starter {starter_name} ...\n')
            (OK, _) = genlib.build_starter(temp_dir, starter_name, script_name, current_run_dir)
            if OK:
                process.write('The file is built.\n')
   ...
   #---------------
    ...
    def build_quercustoa_env_installation_script(self, directory, script_name, current_run_dir):
        '''
        Build the quercusTOA environment creation script on WSL.
        '''
        ...
        # get the YML file to create the quercusTOA environment
        quercustoa_yml_file = f'{app_dir}/{genlib.get_yml_dir()}/{genlib.get_quercustoa_yml_file()}'
        ...
        try:
            with open(script_path, mode='w', encoding='iso-8859-1', newline='\n') as file_id:
                ...
                file_id.write( '}\n')
                file_id.write( '#-------------------------------------------------------------------------------\n')
                file_id.write( 'function remove_quercustoa_env\n')
                file_id.write( '{\n')
                file_id.write( '    echo "$SEP"\n')
                file_id.write(f'    echo "Removing the {genlib.get_app_short_name()} environment ..."\n')
                file_id.write(f'    {miniforge3_condabin_dir}/conda env remove --yes --quiet --name {genlib.get_quercustoa_env_code()}\n')
                file_id.write( '    RC=$?\n')
                ...
                file_id.write( '}\n')
                file_id.write( '#-------------------------------------------------------------------------------\n')
                file_id.write( 'function create_quercustoa_env\n')
                file_id.write( '{\n')
                file_id.write( '    echo "$SEP"\n')
                file_id.write(f'    echo "Creating the {genlib.get_app_short_name()} environment ..."\n')
                file_id.write(f'    {miniforge3_condabin_dir}/conda env create --yes --quiet --name {genlib.get_quercustoa_env_code()} --file {quercustoa_yml_file}\n')
                file_id.write( '    RC=$?\n')
                file_id.write( '    if [ $RC -ne 0 ]; then manage_error conda $RC; fi\n')
                ...
                file_id.write( '#-------------------------------------------------------------------------------\n')
                file_id.write( 'init\n')
                file_id.write( 'remove_quercustoa_env\n')
                file_id.write( 'create_quercustoa_env\n')
                file_id.write( 'end\n')
        except Exception as e:
            error_list.append(f'*** EXCEPTION: "{e}".')
            error_list.append(f'*** ERROR: The file {script_path} can not be created.')
