...
def check_args(args):
    ...
    if args.gff_file is None:
        genlib.Message.print('error', '*** The input GFF file is not indicated in the input arguments.')
        OK = False
    elif not os.path.isfile(args.gff_file):
        genlib.Message.print('error', f'*** The file {args.gff_file} does not exist.')
        OK = False
    ...
    if args.gff_format is None:
        ...
        genlib.Message.print('error', '*** The format of the GFF file has to be GFF3.')
        OK = False
    else:
    ...
    if args.transcripts_geneid_file is None:
        genlib.Message.print('error', '*** The output FASTA file with the RNA sequences is not indicated in the input arguments.')
        OK = False
...
def get_transcripts_geneid(gff_file, gff_format, transcripts_geneid_file, tvi_list):
    ...
    if gff_file.endswith('.gz'):
        try:
            ...
            raise genlib.ProgramException(e, 'F002', gff_file)
    else:
        try:
            gff_file_id = open(gff_file, mode='r', encoding='iso-8859-1')
        except Exception as e:
            raise genlib.ProgramException(e, 'F001', gff_file)
    ...
    if transcripts_geneid_file.endswith('.gz'):
        try:
            ...
            raise genlib.ProgramException(e, 'F004', transcripts_geneid_file)
    else:
        try:
            transcripts_geneid_file_id = open(transcripts_geneid_file, mode='w', encoding='iso-8859-1', newline='\n')
        except Exception as e:
            raise genlib.ProgramException(e, 'F003', transcripts_geneid_file)
    ...
    while record != '':
        ...
        if not record.startswith('#'):
            ...
            try:
                ...
                raise genlib.ProgramException(e, 'F009', os.path.basename(gff_file), record_counter)
        ...
        genlib.Message.print('verbose', f'\rGFF file records: {record_counter:8d} - Genes identifications: {gene_ids_counter:8d}')
